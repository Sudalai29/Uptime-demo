...
resource "aws_iam_role_policy" "task_ssl_certs" {
  name = "${var.prefix}-task-ssl-certs"
  role = aws_iam_role.task.id
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "arn:aws:secretsmanager:${var.aws_region}:${var.aws_account_id}:secret:${var.prefix}/ssl-*"
    }]
  })
}
