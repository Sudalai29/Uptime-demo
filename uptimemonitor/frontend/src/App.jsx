...
function NeuTooltip({ active, payload, label }) {
  ...
  return (
    <div style={{ background: '#f0f4f8', border: 'none', borderRadius: 10, padding: '8px 12px', boxShadow: '4px 4px 12px #b8bec7', fontFamily: 'DM Mono, monospace', fontSize: 11 }}>
      <div style={{ color: '#8896a5', marginBottom: 3 }}>{label}</div>
      {d?.status === 'down'
        ? <div style={{ color: '#fc8181' }}>Offline</div>
        : <div style={{ color: '#38a89d' }}>{d?.ms}ms</div>
      }
    </div>
  )
}
...
function MonitorCard({ item, selected, onSelect, onDelete, onPause }) {
  ...
  const dotColor = st === 'up' ? 'var(--green)' : st === 'down' ? 'var(--red)' : 'var(--muted)'
  const uptimeColor = uptime_pct == null ? 'var(--text)'
    : uptime_pct >= 99 ? 'var(--green)'
    : uptime_pct >= 90 ? 'var(--yellow)'
    : 'var(--red)'
    ...
    <div className={`monitor-card ${selected ? 'selected' : ''}`} onClick={() => onSelect(monitor.id)}>
      <div className="card-top">
        <div className="status-badge">
          <div className="status-badge-dot" style={{ background: dotColor, boxShadow: `0 0 5px ${dotColor}` }} />
          <span style={{ color: dotColor }}>{st.toUpperCase()}</span>
        </div>
        <div className="card-actions" onClick={e => e.stopPropagation()}>
          <span className="card-interval">every {monitor.interval_minutes}m</span>
      ...
      <div className="card-stats">
        <div className="stat-cell">
          <div className="stat-val" style={{ color: uptimeColor }}>{uptime_pct != null ? `${uptime_pct}%` : '—'}</div>
          <div className="stat-lbl">Uptime</div>
        </div>
        <div className="stat-cell">
          <div className="stat-val" style={{ color: 'var(--text)' }}>{avg_response ?? '—'}</div>
        ...
        : (
          <ResponsiveContainer width="100%" height={130}>
            <LineChart data={chartData} margin={{ top: 4, right: 4, bottom: 0, left: 0 }}>
              <XAxis dataKey="time" tick={{ fill: '#8896a5', fontSize: 8, fontFamily: 'DM Mono' }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
              <YAxis tick={{ fill: '#8896a5', fontSize: 8, fontFamily: 'DM Mono' }} tickLine={false} axisLine={false} unit="ms" width={38} />
              <Tooltip content={<NeuTooltip />} />
              <Line type="monotone" dataKey="ms" stroke="#38a89d" strokeWidth={2} dot={false}
                activeDot={{ r: 4, fill: '#38a89d', stroke: '#38a89d' }} connectNulls={false} />
            </LineChart>
          </ResponsiveContainer>
        )
      }
        ...
        {checks.length === 0
          ...
          : checks.slice(0, 15).map((c, i) => (
            <div key={i} className="check-row">
              ...
              <span className="check-code" style={{ color: c.status === 'up' ? 'var(--text)' : 'var(--red)' }}>
  ...
  return (
    <div>
      ...
      <header className="header">
        <div className="header-inner">
          ...
          <div className="status-pill">
            <div
              className={`status-dot ${downCount > 0 ? 'pulse' : ''}`}
              style={{ background: monitors.length === 0 ? 'var(--muted)' : allHealthy ? 'var(--green)' : 'var(--red)', boxShadow: `0 0 6px ${monitors.length === 0 ? 'var(--muted)' : allHealthy ? 'var(--green)' : 'var(--red)'}` }}
            />
          ...
          <div className="header-right">
            ...
            <button className="btn-neu accent" onClick={() => setShowModal(true)}>+ Add Monitor</button>
          </div>
        </div>
      </header>
      ...
      <main className="main">
        ...
        ════════════════════════════════════════════════ */}
        <div className="section" style={{ animation: 'fadeUp 0.4s ease' }}>
          <div className="section-header">
            ...
            {monitors.length > 0 && (
              <div className="section-badge">{monitors.length} registered</div>
            )}
          </div>
        ...
        ════════════════════════════════════════════════ */}
        <div className="section" style={{ animation: 'fadeUp 0.4s ease 0.15s both' }}>
          <div className="section-header">
            <div className="section-title-group">
              <div className="section-icon">📊</div>
              <div>
